...
<html lang="en">
  <head>
    ...
    <title>SGCM - Manutenção de Usuários</title>
      ...
      <div class="container pagina fundo-azul"> <!-- Conteudo -->  
          ...
          <div class="menu">
            <ul class="nav nav-pills">	
			  ...
			  <s:if test="#session.usuario.perfil.id == 1">
			  <li class="active"><s:a action="formListarUsuario.action">Usuários</s:a></li>   
			  </s:if>					  				  
            </ul>
          </div>
          <!-- Mensagens de erro -->
         ...
         <div class="row fundo-filtro-acoes filtros">
            <div class="col-md-12">     
             <form class="form-inline text-center" role="form" id="formCadastro"  method="POST" action="listarUsuario" onsubmit="return validar();">
				...
				<table border="2" class="table table-striped table-bordered table-hover table-condensed">
					...
					<s:iterator value="usuarios">
						<tr>
							...
							<td><s:property value="senha" /></td>
							<td><s:property value="perfil.descricao" /></td>
							<td><s:property value="situacao" /></td>
							<td><s:url action="formAlterarUsuario" var="alt">
									<s:param name="id" value="id" />
								</s:url> <a href='<s:property value="#alt"/>'><img src="img/alterar.png"></a>
							</td>
							<td><s:url action="deletarUsuario" var="del">
									<s:param name="id" value="id" />
 	     ...
 	     <div class="row fundo-filtro-acoes acoes">
            ...
            <form class="form-inline text-center" role="form" id="formCadastro"  method="POST" action="cadastrarUsuario" onsubmit="return validar();">
              <button type="submit" class="btn btn-info"  name="action:cadastrarUsuario"><span class="glyphicon glyphicon-floppy-disk"></span> Novo</button>
              <button type="submit" class="btn btn-default" name="action:cancelarUsuario">Cancelar</button>
			</form>
            </div>
          </div>	         
